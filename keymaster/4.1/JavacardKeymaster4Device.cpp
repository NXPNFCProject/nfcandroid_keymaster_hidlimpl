...
ErrorCode parseWrappedKey(const hidl_vec<uint8_t>& wrappedKeyData, std::vector<uint8_t>& iv,
    ...
    keymaster_error_t error =
            parse_wrapped_key(KeymasterKeyBlob(keyMaterial), &kmIv, &kmTransitKey, &kmSecureKey,
                              &kmTag, &authSet, &kmKeyFormat, &kmWrappedKeyDescription);
...
ErrorCode constructApduMessage(Instruction& ins, std::vector<uint8_t>& inputData,
    ...
    if (UCHAR_MAX < inputData.size() && USHRT_MAX >= inputData.size()) {
        ...
        apduOut.push_back(static_cast<uint8_t>(0x00));
        apduOut.push_back(
                static_cast<uint8_t>(0x00));  // Accepting complete length of output at a time
...
ErrorCode initiateProvision() {
    LOG(INFO) << "Method initiateProvision ";
            ...
            AuthorizationSetBuilder()
                    ...
                    .Authorization(TAG_PURPOSE,
                                   static_cast<keymaster_purpose_t>(
                                           0x7F)) /* The value 0x7F is not present in types.hal */
                    ...
                    .Authorization(TAG_ATTESTATION_ID_MANUFACTURER, manufacturer.data(),
                                   manufacturer.size())
...
Return<ErrorCode> setBootParams() {
    LOG(INFO) << "Method setBootParams ";
    std::vector<uint8_t> verifiedBootKey(32, 0);
    std::vector<uint8_t> verifiedBootKeyHash(32, 0);

    return JavacardKeymaster4Device::setBootParams(GetOsVersion(), GetOsPatchlevel(),
                                                   verifiedBootKey, verifiedBootKeyHash,
...
ErrorCode JavacardKeymaster4Device::provision(const hidl_vec<KeyParameter>& keyParams,
                                              KeyFormat keyFormat,
    ...
    std::vector<uint8_t> masterKey(
            kFakeKeyAgreementKey,
            kFakeKeyAgreementKey + sizeof(kFakeKeyAgreementKey) / sizeof(kFakeKeyAgreementKey[0]));

    /* Subject, AuthorityKeyIdentifier and Expirty time of the root certificate are required by
     * javacard. */
    ...
    if (ErrorCode::OK != (errorCode = constructApduMessage(ins, cborData, apdu))) return errorCode;
    ...
    if ((response.size() > 2)) {
        ...
        std::tie(item, errorCode) = cborConverter.decodeData(
                std::vector<uint8_t>(response.begin(), response.end() - 2), true);
    }
    return errorCode;
}

ErrorCode JavacardKeymaster4Device::setBootParams(uint32_t osVersion, uint32_t osPatchLevel,
                                                  const std::vector<uint8_t>& verifiedBootKey,
                                                  std::vector<uint8_t>& verifiedBootKeyHash,
                                                  keymaster_verified_boot_t kmVerifiedBoot,
                                                  bool deviceLocked) {
    cppbor::Array array;
    std::vector<uint8_t> apdu;
    std::vector<uint8_t> response;
...
JavacardKeymaster4Device::JavacardKeymaster4Device(SecurityLevel securityLevel)
    : softKm_(new ::keymaster::AndroidKeymaster(
              ...
              kOperationTableSize)),
      softKm4_0_(new keymaster::V4_0::ng::AndroidKeymaster4Device(securityLevel)),
      oprCtx_(new OperationContext()) {
    LOGD_JC("constructed");
}

JavacardKeymaster4Device::~JavacardKeymaster4Device() {}
// TODO: Remove socket related comments ??
    ...
    if (ret == ErrorCode::SECURE_HW_COMMUNICATION_FAILED) {
        ...
        if ((ret == ErrorCode::OK) && (resp.size() > 2)) {
            ...
            std::tie(item, ret) = cborConverter_.decodeData(
                    std::vector<uint8_t>(resp.begin(), resp.end() - 2), true);
            if (item != nullptr) {
...
#define KM_USE_JAVACARD "debug.use_javacard"
bool useJavaCard() {
#ifdef USE_SW_KM4
    return android::base::GetBoolProperty(KM_USE_JAVACARD, false);
#else
    return android::base::GetBoolProperty(KM_USE_JAVACARD, true);
#endif
}

Return<void> JavacardKeymaster4Device::getHmacSharingParameters(
    ...
    if (!use_java_card) {
        ...
        if (errorCode == ErrorCode::SECURE_HW_COMMUNICATION_FAILED) {
            ...
            static_assert(sizeof(response.params.nonce) == params.nonce.size(),
            ...
            if ((errorCode == ErrorCode::OK) && (cborData.size() > 2)) {
                // Skip last 2 bytes in cborData, it contains status.
                std::tie(item, errorCode) = cborConverter_.decodeData(
                        std::vector<uint8_t>(cborData.begin(), cborData.end() - 2), true);
                if (item != nullptr) {
...
Return<void> JavacardKeymaster4Device::computeSharedHmac(
...
Return<void> JavacardKeymaster4Device::verifyAuthorization(
...
Return<void> JavacardKeymaster4Device::importWrappedKey(
...
#ifdef USE_SW_KM4
    softKm4_0_->importWrappedKey(wrappedKeyData, wrappingKeyBlob, maskingKey, unwrappingParams,
    ...
    array.add(biometricSid); /* TODO if biometricSid optional if user not sent this don't encode
                                this cbor format */
    std::vector<uint8_t> cborData = array.encode();
        ...
        if (item != nullptr) {
            if (!cborConverter_.getMultiBinaryArray(item, 1, temp)) {
                ...
                if (readDataFromFile(ROOT_RSA_CERT, rootCert)) {
                    ...
                    LOGE_JC("No root certificate found , returning only Public Key Attestation "
                            "cert received from Applet");
                    certChain.resize(temp.size());
                    for (int i = 0; i < temp.size(); i++) {
                        certChain[i] = temp[i];
                    }
...
Return<void> JavacardKeymaster4Device::begin(KeyPurpose purpose, const hidl_vec<uint8_t>& keyBlob,
                                             ...
                                             begin_cb _hidl_cb) {
#ifdef USE_SW_KM4
    return softKm4_0_->begin(purpose, keyBlob, inParams, authToken, _hidl_cb);
#else
    ErrorCode errorCode = ErrorCode::UNKNOWN_ERROR;
...
Return<void> JavacardKeymaster4Device::update(uint64_t operationHandle,
                                              const hidl_vec<KeyParameter>& inParams,
    ...
    if (response.error == KM_ERROR_OK) {
        ...
        auto sendDataCallback = [&](std::vector<uint8_t>& data, bool) -> ErrorCode {
            ...
            // aligned), if there is ASSOCIATED_DATA present in KeyParameters. Then we need to make a
            // call to javacard Applet.
        ...
        if (ErrorCode::OK ==
            (errorCode = oprCtx_->update(operationHandle, std::vector<uint8_t>(input),
                                         sendDataCallback))) {
...
Return<void> JavacardKeymaster4Device::finish(uint64_t operationHandle,
                                              const hidl_vec<KeyParameter>& inParams,
                                              const hidl_vec<uint8_t>& input,
                                              ...
                                              finish_cb _hidl_cb) {
#ifdef USE_SW_KM4
    softKm4_0_->finish(operationHandle, inParams, input, signature, authToken, verificationToken,
                       _hidl_cb);
#else
    ErrorCode errorCode = ErrorCode::UNKNOWN_ERROR;
    ...
    if (response.error == KM_ERROR_OK) {
        ...
        auto sendDataCallback = [&](std::vector<uint8_t>& data, bool finish) -> ErrorCode {
            ...
            // update and finish calls. aadTag is used to check if ASSOCIATED_DATA is already sent in
            // update call. If addTag is true then skip ASSOCIATED_DATA from keyparams in finish
            // call.
            // Convert input data to cbor format
            array.add(operationHandle);
            if (finish) {
        ...
        if (ErrorCode::OK ==
            (errorCode = oprCtx_->finish(operationHandle, std::vector<uint8_t>(input),
                                         sendDataCallback))) {
...
Return<::android::hardware::keymaster::V4_1::ErrorCode> JavacardKeymaster4Device::deviceLocked(
        bool passwordOnly, const VerificationToken& verificationToken) {
#ifdef USE_SW_KM4
    LOGD_JC("using SW impl: UNIMPLEMENTED"
            << "Implemented param1 =" << passwordOnly
            << " param2 = " << verificationToken.challenge);
    ...
    if ((ret == ErrorCode::OK) && (cborOutData.size() > 2)) {
        ...
        std::tie(item, errorCode) =
                cborConverter_.decodeData<::android::hardware::keymaster::V4_1::ErrorCode>(
                        std::vector<uint8_t>(cborOutData.begin(), cborOutData.end() - 2), true);
    }
    return errorCode;
#endif

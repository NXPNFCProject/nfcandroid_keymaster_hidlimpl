...
ErrorCode OperationContext::validateInputData(uint64_t operHandle, Operation opr,
    ...
    if (opr == Operation::Finish) {
        ...
        // MAX_ALLOWED_INPUT_SIZE then combine both the data in a single buffer. This helps in making
        // sure that no data is left out in the buffer after finish opertion.
...
ErrorCode OperationContext::getBlockAlignedData(uint64_t operHandle, uint8_t* input,
    ...
    if (dataToSELen > 0) {
        ...
        // consumed is 0. That means all the data to be send to SE is consumed from the buffer.
    ...
    if (Algorithm::AES == operationTable[operHandle].info.alg ||
        ...
        if (operationTable[operHandle].info.purpose == KeyPurpose::DECRYPT ||
            ...
            if (finish) {
                ...
                // should be more than 256 and for EC it should not be more than 32. This validation
                // is already happening in validateInputData function. Just for safety sake we are
                // checking the length to MAX_BUF_SIZE.

...
class JavacardKeymaster4Device : public IKeymasterDevice {
  public:
    ...
    static ErrorCode setBootParams(uint32_t osVersion, uint32_t osPatchLevel,
                                   const std::vector<uint8_t>& verifiedBootKey,
                                   std::vector<uint8_t>& verifiedBootKeyHash,
                                   keymaster_verified_boot_t kmVerifiedBoot, bool deviceLocked);

    // Provision Method
    /* Reference for vendor to provision the javacard. This should happen only once at the time of
     * production.*/
    static ErrorCode provision(const hidl_vec<KeyParameter>& keyParams, KeyFormat keyformat,

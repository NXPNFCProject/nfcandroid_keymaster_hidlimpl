...
class SecureElementCallback : public ISecureElementHalCallback {
  public:
    ...
    bool isClientConnected() { return mSEClientState; }
...
bool AppletConnection::connectToSEService() {
    ...
    uint8_t retry = 0;
    bool status = false;
    while ((mSEClient == nullptr) &&
           retry++ < MAX_GET_SERVICE_RETRY) {  // How long should we try before giving up !
        mSEClient = ISecureElement::tryGetService("eSE1");

        if (mSEClient == nullptr) {
            LOG(ERROR) << "failed to get eSE HAL service : retry after 1 sec , retry cnt = "
                       << android::hardware::toString(retry);
        } else {
...
bool AppletConnection::openChannelToApplet(std::vector<uint8_t>& resp) {
    ...
    mSEClient->openLogicalChannel(
            kAppletAID, 00, [&](LogicalChannelResponse selectResponse, SecureElementStatus status) {
...
bool AppletConnection::transmit(std::vector<uint8_t>& CommandApdu, std::vector<uint8_t>& output) {
    ...
    mSEClient->transmit(cmd, [&](hidl_vec<uint8_t> result) {
        output = result;
        LOG(INFO) << "recieved response size = " << ::android::hardware::toString(result.size())
                  << " data = " << result;
    });
    return true;
}
